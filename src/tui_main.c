...
static void ged_cursor_move_relative(Ged_cursor *tc, Usz field_h, Usz field_w, Isz delta_y, Isz delta_x)
{
...
staticni void draw_grid_cursor(
    WINDOW *win,
    int draw_y,
    int draw_x,
    int draw_h,
...
staticni void draw_glyphs_grid(
    WINDOW *win,
    int draw_y,
    int draw_x,
    int draw_h,
...
staticni bool ged_resize_grid_snap_ruler(
    Field *field,
    Mbuf_reusable *mbr,
    Usz ruler_y,
    Usz ruler_x,
...
staticni bool portmidi_find_device_id_by_name(
    char const *name,
    Usz namelen,
    PmError *out_pmerror,
    PmDeviceID *out_id)
...
static bool portmidi_find_name_of_device_id(PmDeviceID id, PmError *out_pmerror, oso **out_name)
{
    *out_pmerror = portmidi_init_if_necessary();
    if (*out_pmerror)
        return false;
...
staticni void send_midi_3bytes(Oosc_dev *oosc_dev, Midi_mode const *midi_mode, int status, int byte1, int byte2)
{
...
static void send_midi_chan_msg(
    Oosc_dev *oosc_dev,
...
static void send_midi_byte(Oosc_dev *oosc_dev, Midi_mode const *midi_mode, int x)
{
...
staticni void //
send_midi_note_offs(Oosc_dev *oosc_dev, Midi_mode *midi_mode, Susnote const *start, Susnote const *end)
{
    for (; start != end; ++start) {
#if 0
...
static void send_num_message(Oosc_dev *oosc_dev, char const *osc_address, I32 num)
{
    if (!oosc_dev)
        return;
    I32 nums[1];
...
staticni void apply_time_to_sustained_notes(
    Oosc_dev *oosc_dev,
    Midi_mode *midi_mode,
    double time_elapsed,
    Susnote_list *susnote_list,
...
staticni void send_output_events(
    Oosc_dev *oosc_dev,
    Midi_mode *midi_mode,
    Usz bpm,
    Susnote_list *susnote_list,
...
static bool ged_is_using_osc_udp(Ged *a)
{
    return (bool)a->oosc_dev;
}
static bool ged_set_osc_udp(Ged *a, char const *dest_addr, char const *dest_port)
{
    ged_clear_osc_udp(a);
    if (dest_port) {
...
staticni void clear_and_run_vm(
    Glyph *restrict gbuf,
    Mark *restrict mbuf,
    Usz height,
    Usz width,
...
staticni Isz
scroll_offset_on_axis_for_cursor_pos(Isz win_len, Isz cont_len, Isz cursor_pos, Isz pad, Isz cur_scroll)
{
...
staticni void ged_set_window_size(Ged *a, int win_h, int win_w, int softmargin_y, int softmargin_x)
{
...
staticni void ged_draw(Ged *a, WINDOW *win, char const *filename, bool use_fancy_dots, bool use_fancy_rulers)
{
...
staticni bool ged_try_selection_clipped_to_field(Ged const *a, Usz *out_y, Usz *out_x, Usz *out_h, Usz *out_w)
{
...
static Usz view_to_scrolled_grid(Usz field_len, Usz visual_coord, int scroll_offset)
{
    if (field_len == 0)
        return 0;
    if (scroll_offset < 0) {
...
ORCA_OK_IF_UNUSED staticni void ged_mouse_event(Ged *a, Usz vis_y, Usz vis_x, mmask_t mouse_bstate)
{
...
static void push_plainorfancy_menu(int menu_id, char const *title, bool initial_fancy)
{
...
#ifdef FEAT_PORTMIDI
    if (t->ged.midi_mode.any.type == Midi_mode_type_null && osolen(portmidi_output_device)) {
    ...
    switch (t->ged.midi_mode.any.type) {
        ...
        case Midi_mode_type_portmidi: {
            ...
            if (!portmidi_find_name_of_device_id(
                    t->ged.midi_mode.portmidi.device_id,
                    &pmerror,
...
staticni bool tui_suggest_nice_grid_size(Tui *t, int win_h, int win_w, Usz *out_grid_h, Usz *out_grid_w)
{
    int softmargin_y = t->softmargin_y, softmargin_x = t->softmargin_x;
    int ruler_spacing_y = (int)t->ged.ruler_spacing_y, ruler_spacing_x = (int)t->ged.ruler_spacing_x;
...
staticni bool tui_suggest_tight_grid_size(Tui *t, int win_h, int win_w, Usz *out_grid_h, Usz *out_grid_w)
{
    int softmargin_y = t->softmargin_y, softmargin_x = t->softmargin_x;
...
staticni void plainorfancy_menu_was_picked(Tui *t, int picked_id, bool *p_is_fancy, U32 pref_touch_flag)
{
...
staticni bool tui_restart_osc_udp_if_enabled_diderror(Tui *t)
{
    ...
    if (t->osc_output_enabled && t->osc_port) {
        error = !ged_set_osc_udp(&t->ged, osoc(t->osc_address) /* null ok here */, osoc(t->osc_port));
    } else {
...
staticni void tui_adjust_term_size(Tui *t, WINDOW **cont_window)
{
    ...
    ged_set_window_size(&t->ged, content_h, content_w, t->softmargin_y, t->softmargin_x);
}
                        ...
                        switch (qform_id(qf)) {
                            case Open_form_id: {
                                ...
                                bool added_hist = undo_history_push(
                                    ...
                                    &t->ged.field,
                                    t->ged.tick_num);
                                Field_load_error fle = field_load_file(osoc(temp_name), &t->ged.field);
...
#undef OPTFAIL
    ...
    wtimeout(stdscr, 0);
    int cur_timeout = 0;
    Usz brackpaste_starting_x = 0, brackpaste_y = 0, brackpaste_x = 0, brackpaste_max_y = 0,
        brackpaste_max_x = 0;
    ...
    if (!grid_initialized) {
        ...
        if (explicit_initial_grid_size ||
            !tui_suggest_nice_grid_size(&t, t.ged.win_h, t.ged.win_w, &new_field_h, &new_field_w)) {
    ...
    mbuf_reusable_ensure_size(&t.ged.mbuf_r, t.ged.field.height, t.ged.field.width);
    ged_make_cursor_visible(&t.ged);
    ...
    switch (key) {
        case ERR: { // ERR indicates no more events.
            ...
            if (ged_is_draw_dirty(&t.ged) || qnav_stack.occlusion_dirty) {
                werase(cont_window);
                ged_draw(&t.ged, cont_window, osoc(t.file_name), t.fancy_grid_dots, t.fancy_grid_rulers);
...
#undef DEADTIME
        ...
        if (key >= CHAR_MIN && key <= CHAR_MAX) {
            ...
            if (key != ' ') {
                ...
                if (!orca_is_valid_glyph((Glyph)key))
                    cleaned = '.';
                if (brackpaste_y < t.ged.field.height && brackpaste_x < t.ged.field.width) {
                    gbuffer_poke(
                        t.ged.field.buffer,
                        t.ged.field.height,
                        t.ged.field.width,
    ...
    switch (key) {
        ...
        case CTRL_PLUS('v'):
            if (t.use_gui_cboard) {
                bool added_hist = undo_history_push(&t.ged.undo_hist, &t.ged.field, t.ged.tick_num);
